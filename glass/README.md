...
- **Built-in browser** — a resizable split-view browser (globe button in the header) for previewing what agents build: back/forward/reload, URL bar with smart normalization (`localhost:3000` just works), quick localhost links, and a camera button that screenshots the page and attaches it to your next message (sent to the agent via the SDK's image support)
...
- **Built-in browser.** Implemented with Electron's `<webview>` tag (isolated `persist:glass-browser` partition, no Node integration, popups stay in-view via `setWindowOpenHandler` on the guest). Screenshots are captured in the main process from the guest `webContents` and sent as base64 images on `agent.send({ text, images })`. Screenshots can't be queued — image payloads don't persist on the message queue.
